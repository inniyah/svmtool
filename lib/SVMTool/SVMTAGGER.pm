...
sub randomize_sentences{
    ...
    while (defined(my $line = $CORPUS->getline())) {
        ...
        if (COMMON::end_of_sentence($word) || ($word eq $COMMON::SMARK)) {
           ...
           if (($word eq $COMMON::SMARK) and (COMMON::end_of_sentence($prev_word))) { print $last $sentence; }
           else {
              $rn = rand($Xtrain + $Xval + $Xtest);
              if ($rn < $Xtrain) { print $TRAIN $sentence; $ntrain++; $wtrain += $nwords; $last = $TRAIN; }
              elsif (($rn >= $Xtrain) && ($rn < $Xtrain + $Xval)) { print $VAL $sentence; $nval++; $wval += $nwords; $last = $VAL; } 
              elsif (($rn >= $Xtrain + $Xval) && ($rn <= $Xtrain + $Xval + $Xtest)) { print $TEST $sentence; $ntest++;  $wtest += $nwords; $last = $TEST; }
	   }
           @sentence_list = ();
        }
        $prev_word = $word;
...
sub create_dictionary
{
   ...
   srand();
   my $DICT = $dict.".".rand($COMMON::NRAND);
...
sub get_sentence_info
{
   ...
   return [COMMON::end_of_sentence($lastw)? $lastw : $COMMON::SMARK];
}
...
sub do_attribs_kn
{
   ...
   while (!$stop) {
      my $stdin;
      my $input_LR;
      ($stdin, $input_LR, $stop) = ENTRY::read_sentence_file($CORPUS);
      ...
      while ($iter < $SIZE) {
         ...
         my @columns = @{$columns}; shift(@columns); shift(@columns);
         $rwindow->push($word, ($wordpos ne $COMMON::emptypos)? $wordpos : $rdict->tag($word), $config->{wlen} - 1, \@columns);
...
sub do_attribs_kn_unk
{
   ...
   my $dict = $model.".".$COMMON::DICTEXT.".".rand($COMMON::NRAND);
   my $DICT = $dict.".R";
   my $rdict;
   my ($trainsize, $nchunks) = SVMTAGGER::get_train_size($corpus, $config->{X});
   if ($verbose > $COMMON::verbose2) { print STDERR "X = ", $config->{X}, " :: CHUNKSIZE = $trainsize :: CHUNKS = $nchunks\n"; }
   my $chunk = 0;
   my $ITER = 0;
   my $stop = 0;
   while (!$stop) {
      ...
      while ($iter < $SIZE) {
         ...
         if ((($ITER+$iter) % $trainsize) == 0) {
            if ($verbose > $COMMON::verbose2) { print STDERR "STARTING CHUNK $chunk : reading word -> ", $ITER + $iter, "\n"; }
   	        elsif ($verbose > $COMMON::verbose1) { print STDERR "."; }
            ...
            if ($mode != $COMMON::mode3) {
               ...
               if ($config->{R} ne "") { $N = SVMTAGGER::repair_dictionary($dict, $config->{R}, $DICT); }
               else { $N = SVMTAGGER::repair_dictionary_H($dict, $DICT, $config->{Dratio}); }
               system "mv $DICT $dict";
            }
            else {  #dictionary enriching [unsupervised]
               if ($config->{LEX} ne "") { SVMTAGGER::enrich_dictionary($dict, $config->{LEX}); }
	        }
            #backup lexicon dictionary enriching
            if ($config->{BLEX} ne "") { SVMTAGGER::enrich_dictionary($dict, $config->{BLEX}); }
            $rdict = new DICTIONARY($dict, $fambp, $funkp);
            system "rm -f $dict";
            if ($verbose > $COMMON::verbose2) { print STDERR " -> number of words $rdict->{nwords} [#REPAIRINGS = $N]\n"; }
  	        $chunk++;
         }
         #shift sliding window one position left
         $rwindow->lshift(1);
         ...
         $iter++;
         if ($verbose > $COMMON::verbose1) { COMMON::show_progress_stdout($ITER + $iter, $COMMON::progress1, $COMMON::progress2); }
      }
      my $i = $config->{wlen} - $config->{wcore} - 1;
      while ($i > 0) { #process last words     
         $rwindow->lshift(1);
...
sub do_learn
{
   ...
   while ($ID < scalar(@{$rambpos})) {
      ...
      my $currenttraindsf = "$model.$modext.$dirext.$COMMON::smplext.$ID.$COMMON::DSFEXT.".rand($COMMON::NRAND);
...
sub get_train_size
{
   ...
   my $CORPUS = new IO::File("< $corpus") or die "Couldn't open input file: $corpus\n";
   while (defined(my $line = $CORPUS->getline())) {
      my $entry = COMMON::trim($line); 
      if (($entry ne "") and ($entry ne $COMMON::SMARK)) {
         $nwords++;
         my @l = split($COMMON::in_valseparator, $entry);
         if (exists($WORDS{$l[0]})) { $WORDS{$l[0]} += 1; }
         else { $WORDS{$l[0]} = 1; $ndwords++; }
   	  }
   }
   $CORPUS->close();
   ...
   while (defined(my $line = $CORPUS->getline()) and ($ndwords2 < $meeting)) {
   	  my $entry = COMMON::trim($line); 
   	  if ($entry ne $COMMON::SMARK) {
         $nwords2++;
         my @l = split($COMMON::in_valseparator, $entry);
         if (exists($WORDS2{$l[0]})) { $WORDS2{$l[0]} += 1; }
         else { $WORDS2{$l[0]} = 1; $ndwords2++; }
   	  }
   }
   $CORPUS->close();
...
sub SVMT_load
{
   ...
   if ($blexicon ne "") { #joining backup lexicon
      srand();
      my $xpnddict = $COMMON::DICTEXT.".".rand($COMMON::NRAND);
...
sub adjust_C
{
    ...
    while ($depth < $MAX_DEPTH){
       ...
       while (((sprintf("%.4f", $MAX_C - $C) + 0) >= 0) && ($C >= 0)) {
          ...
          if (-e "$model.$modext.$dirext.$COMMON::MRGEXT") {
             system "cp $model.$modext.$dirext.$COMMON::MRGEXT $model.$modext.$dirext.$COMMON::mrgext" == 0 or die "system failed: $?";
          } 
          ...
          SVMTAGGER::merge_models($model, $modext, $dirext, $smplmap, $unksmplmap, $config, $COMMON::MRGEXT, $unknown, $report, $verbose);
          srand();
          my $output_file = "$model.$modext.$dirext.$COMMON::clsfext.".rand($COMMON::NRAND);
...
sub adjust_C_CV
{
    #description _ responsible for tuning C parameter for SVMT models (under cross validation).
    #param1  _ model name
    #param2  _ training set filename
    #param3  _ dictionary object reference
