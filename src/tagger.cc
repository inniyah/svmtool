...
tagger::tagger(char *model)
{
	...
	taggerUFilter = 0;
								 //modstrat //0;
	taggerStrategy = STRA_1P_DEFAULT;
	taggerWinIndex = -1;
	taggerWinLength = -1;
...
void tagger::taggerLoadModelsForTagging()
{
	...
	weightUnk = taggerCreateWeightUnkArray(name);
								 ...
								 //modstrat 1
	if ( taggerStrategy == STRA_2P_RELABELING
	...
	taggerModelRunning = &taggerModelList[0];
								 ...
								 //modstrat 0)
	if (taggerStrategy == STRA_1P_DEFAULT )
		taggerLoadModels(taggerModelRunning,0);
								 //modstrat 2)
	else if (taggerStrategy == STRA_1P_UNSUPERVISED )
		taggerLoadModels(taggerModelRunning,3);
								 //modstrat 4)
	else if (taggerStrategy ==  STRA_1P_ROBUST_UNK )
	{
		taggerLoadModels(taggerModelRunning,0);
		taggerLoadModels(&taggerModelList[1],2);
	}
								 //modstrat 5)
	else if (taggerStrategy == STRA_1P_VERY_ROBUST_UNK )
		taggerLoadModels(taggerModelRunning,4);
								 //modstrat 1)
	else  if (taggerStrategy == STRA_2P_RELABELING )
	{
...
void tagger::taggerGenerateScore(nodo *elem,int direction)
{
	...
	if (i!=HASH_FAIL)
	{
		...
		{
								 //wr2;
			weightRep = taggerModelRunning->wr2;
	...
	else
	{
		...
		{
								 //wrUnk2;
			weightRep = taggerModelRunning->wrUnk2;
			//bias =	taggerModelRunning->biasUnk2; //taggerBiasUnk2;
		}
		else
		{						 //wrUnk;
			weightRep = taggerModelRunning->wrUnk;
