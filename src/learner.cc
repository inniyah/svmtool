...
/*
 * Parmetros:
 * wrd		char *	Palabra de ejemplo que se est tratando.
 * numModel	int	Modelo que estamos entrenando.
 * direction	int	Direccin en la cual se realiza el entrenamiento
			(Derecha a izquierda o izquierda a derecha).
 * Known_or_Unknown	int	Si se est entrenando para palabras conocidas
				o desconocidas.
 * pos	char *	La etiqueta morfosintctica que estamos entrenando.
 * samplePos	char *	Etiqueta morfosintctica del ejemplo  que estamos tratando.
 * features	char *	Lista de atributos generados para el ejemplo.
 * d	dictionary *	Diccionario que se est usando para el entrenamiento.
 * nNeg	int *	Apuntador al nmero de palabras seleccionadas como ejemplos negativos.
 * nPos	int *	Apuntador al nmero de palabras seleccionadas como ejemplos positivos.
 *
 * Este mtodo puede usarse para seleccionar ejemplos para palabras conocidas
 */
void learner::learnerPushSample(char *wrd,int numModel,int direction, int Known_or_Unknown,char *pos, char *samplePos, char *features,dictionary *d, int *nNeg, int *nPos)
{
	...
	//Se obtiene la lista de posibles etiquetas morfosintcticas para
	//la palabra wrd.
...
/*
 * Parmetros:
 * wrd	Char *	Palabra del ejemplo que se est tratando.
 * numModel	Int	Modelo que estamos entrenando.
 * direction	Int	Direccin en la cual se realiza el entrenamiento
			(Derecha a izquierda o izquierda a derecha).
 * Known_or_Unknown	Int	Si se est entrenando para palabras conocidas
 ...
 * nNeg	int *	Apuntador al nmero de palabras seleccionadas como ejemplos negativos.
 * nPos	int *	Apuntador al nmero de palabras seleccionadas como ejemplos positivos.
 * Este mtodo se encarga de seleccionar ejemplos para palabras desconocidas.
*/
...
/**************************************************************/
/*
 * Este mtodo recibe como parmetros un apuntador a un fichero (f)
 * y el apuntador a un objeto del tipo mapping. El objetivo de este
 * mtodo es leer los atributos generados para un ejemplo del fichero
...
char *learner::learnerCreateFeatureString(FILE *f,mapping *m)
{
	...
	//identificadores numricos de cada atributo
	while (ret1>0 && !feof(f))
	{
		ret1 = readTo(f,' ','\n',str);
 ...
 * Parmetros:
 * d	dictionary*	Diccionario que se est usando para el entrenamiento.
 * m	Mapping*	Mapping entre atributos e identificadores numricos
 * f	FILE*	Apuntador al fichero de ejemplos
 * pos	char *	La etiqueta morfosintctica que estamos entrenando.
 * numModel	int	Modelo que estamos entrenando.
 * direction	int	Direccin en la cual se realiza el entrenamiento (Derecha a
			izquierda o izquierda a derecha).
 * K_or_U	int	Si se est entrenando para palabras conocidas o desconocidas.
 * nNeg	int *	Apuntador al nmero de palabras seleccionadas como ejemplos negativos.
 * nPos	int *	Apuntador al nmero de palabras seleccionadas como ejemplos positivos.
 *
 * Este mtodo lee el fichero en el cual estn contenidos los ejemplos seleccionados
 ...
 * f	FILE*	Apuntador al fichero de ejemplos
 * numModel	Int	Modelo que estamos entrenando.
 * LR_or_RL	Int	Direccin en la cual se realiza el entrenamiento
 * 			(Derecha a izquierda o izquierda a derecha).
 * K_or_U	Int	Si se est entrenando para palabras conocidas o desconocidas.
 * d	dictionary*	Diccionario que se est usando para el entrenamiento.
 * lPosToTrain	simpleList *	Lista de etiquetas morfosintcticas a entrenar
 *
 * Este mtodo es el encargado de realizar el aprendizaje. Prepara las opciones de
 * ejecucin  para SVM-light.  Construye el mapping mediante los datos contenidos
 * en el fichero de entrada. Crea un depsito de pesos (weightRepository) y
 ...
 * Para cada etiqueta morfosintctica de la lista lPosToTrain, se
 * llama al mtodo learnerDressNakedSetTrain para conseguir los ficheros
 ...
 * Una vez procesadas todas las etiquetas morfosintcticas a entrenar, el depsito
 * de pesos y el hashing de sesgos contienen todos los datos del modelo y se escriben
 * en disco mediante los mtodos weightRepository.wrWriteHash para los sesgos y
...
void learner::learnerDoLearn(FILE *f,int numModel,int LR_or_RL,int K_or_U,dictionary *d,simpleList *lPosToTrain)
{
	...
	//Preparamos las opciones con que se ejecutar svm-light
	char options[100]="";
	...
	//Creamos el depsito de pesos y el hash de sesgos
	hash_t *b = new hash_t;
	hash_init(b,30);
	...
	for (int ret=1; ret>=0; ret=lPosToTrain->next())
	{
		...
		//Se insertan los valores obtenidos del entrenamiento en el depsito de pesos
	...
	//Modificacin 180705: Filtrado de pesos
								 //ADD 180705
	if ( K_or_U == KNOWN ) wr->wrWrite(fileName, KFILTER);
								 //ADD 180705
	else wr->wrWrite(fileName, UFILTER);
	...
	fprintf (fwr,"BIASES ");
	wr->wrWriteHash(b,fwr,' ');	 //Escribir biases en fichero de depsito de pesos
	fclose(fwr);
...
/*******************************************************/
/*
 * Recibe como parmetro el nombre del fichero de entrenamiento
 * (trainingFileName), el diccionario para palabras conocidas (dKnown),
 * el nmero del modelo a entrenar (numModel), la direccin en la cual
 * se realiza el entrenamiento (direction), el nmero de frases del
 * corpus (numSent), el nmero de palabras del corpus (numWords) y
 * el nmero de fragmentos en que se ha de dividir el corpus para
 * entrenar palabras desconocidas (numChunks).
 *
 * Este mtodo selecciona ejemplo y realiza el entrenamiento del modelo
 */
...
/*
 * Recibe como parmetros: wrd un string con una palabra,
 * pos un string con una etiqueta, Known_or_Unknown si la
 * palabra es conocida o desconocida. Si la palabra es conocida
 * devuelve TRUE si la etiqueta pos est en la lista de etiquetas
 * morfosintcticas ambiguas. Si la palabra es desconocida
 * devuelve TRUE si la etiqueta pos pertenece a lista de posibles
 * categoras morfosintcticas para palabras desconocidas. En cualquier
...
simpleList *learner::learnerGetPotser(char *wrd, int Known_or_Unknown, dictionary *d)
{
	...
	//para la palabra wrd  y mira si existen en la lista de categoras
	//morfosintcticas ambiguas. Si existen en la lista de etiquetas ambiguas
	//las aade a la lista de salida, en caso contrario si no esta es posible
	//etiqueta ambigua y es la etiqueta ms frecuente se devuelve una lista
	//slo con la etiqueta ms frecuente.
	if (Known_or_Unknown==KNOWN)
	{
		if (w != HASH_FAIL)
		{
			...
			while (ret >= 0 && stop == FALSE)
			{
				...
				if (HASH_FAIL!=hash_lookup(learnerAMBP_H,pInfoDict->txt))
				{
					...
					lout->add(ptr);
								 ...
								 //si es most frequent tag
				else if ( strcmp(d->getMFT(w),pInfoDict->txt)==0 )
				{
	...
	//categoras morfosintcticas posibles desconocidas.
	else  if (Known_or_Unknown==UNKNOWN)
	{
...
/*
 * Recibe como parmetros:
 * un apuntador al depsito de pesos (wr),
 ...
 * cada uno de los atributos ledos gracias al mapping (m).
 * Y aadiendo la correspondiente pareja etiqueta/atributo
 * al depsito de pesos wr mediante el mtodo wrAdd.
 *
 * Se devuelve el apuntador al depsito de pesos (weightRepository)
...
void learner::learnerDestroyBias(hash_t *h)
{
	...
	//Recorre cada lista de sinnimos del hash eliminando su contenido
 ...
 * Este mtodo lee el sesgo del fichero y lo aade (hash_insert)
...
/*
 * Recibe como parmetro el nombre del fichero de configuracin (train).
...
void learner::learnerRun(char *train)
{
	...
	init_stack(&DO);
	//Leemos el fichero de configuracin
	read_config_file(train);
	//obtener tamao del corpus
	...
	dictionary *d = new dictionary(TRAINSET, 0,0);
								 // "/mnt/hda4/pfc/WSJ.200");
	if (R!=NULL) d->dictRepairFromFile(R);
	...
	//creamos el fichero de configuracin de la ventana
	...
	if ( verbose_svmtool == TRUE )
	{
		...
		showTime ("* SVMTlearn Time",
								 //CLK_TCK,
			((double)(end-start))/CLOCKS_PER_SECOND - time_svmlight,
 ...
 * el diccionario con la informacin necesaria para el clculo de features (d)
 * y  la direccin en que se recorre el corpus (direction).
 * Recorre la lista featureList y ejecuta los mtodos necesarios
 * de la ventana (swindow) para generar los atributos y que al final
 * de la ejecucin de este mtodo esten apilados en stk.
 */
...
/*
 * El objetivo de este mtodo es seleccionar o descartar una palabra para
 * realizar en el entrenamiento de palabras desconocidas, calcular respectivos
 * atributos e insertar esta informacin en el fichero de ejemplos correspondiente.
 */
...
/************************************************************/
/*
 * Ejecuta SVM-light. Recibe como parmetros cuatro cadenas de
 * caracteres: svmdir es el directorio en el que se encuentra SVM-light,
 * options son las opciones con que se lanzar SVM-light, posFile el nombre
 * del fichero de ejemplos usado como entrada para la herramienta
 * de Joachims y, por ltimo, outFile que es el nombre del fichero
 * de salida.  Esta funcin devuelve 0.
 */
...
/*
 * Calcularemos el nmero de fragmentos en el que se ha de dividir
 * el corpus para conseguir un porcentaje de palabras desconocidas
 * determinado. Lo parmetros de entrada son el nombre del fichero
 * de entrenamiento (trainingFileName), el porcentaje de palabras
 * desconocidas deseado (percentage) y el nmero de frases del corpus
 * (nSentences). El valor devuelto es un entero indicando el nmero de
 * fragmentos. Si el nmero de fragmentos calculado es mayor que el nmero
 * de frases que contiene el corpus de entrenamiento se devolver como el
 * nmero de frases (nSentences).
 */
