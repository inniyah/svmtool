...
int main(int argc, const char * argv[])
{
	...
	int N = 10; // Input: number of source points
	...
	double h=0.01; // Input: source bandwidth or scale, must be a positive number, greater than zero
	double r = 0; // Input: derivative order, must be a positive integer, or zero
	double epsilon = 1e-6; // Input: error
	double D_fast[M]; // Output: array of M elements, density derivative evaluated at each target point
	double D_direct[M]; // Output: array of M elements, density derivative evaluated at each target point
	for (int i = 0; i < M; ++i) D_fast[i] = D_direct[i] = 0.0; // Initialize results array

	FastUnivariateDensityDerivative * pUDD_fast = new FastUnivariateDensityDerivative(N, M, X, Y, h, r, epsilon, D_fast);
	pUDD_fast->Evaluate();
	delete pUDD_fast;

	printf("Dfast = { ");
	for (int i = 0; i < M; ++i) printf("%lf ", D_fast[i] / pow(10.0, r));
	printf("}\n");

	UnivariateDensityDerivative * pUDD_direct = new UnivariateDensityDerivative(N, M, X, Y, h, r, D_direct);
	pUDD_direct->Evaluate();
	delete pUDD_direct;

	printf("Ddirect = { ");
	for (int i = 0; i < M; ++i) printf("%lf ", D_direct[i] / pow(10.0, r));
	printf("}\n");

	double Q = 1.0 / (sqrt(2.0 * M_PI) * pow(h, r + 1.0));
	double max_error = fabs(D_direct[0] - D_fast[0]) / Q;
	for (int i = 1; i < M; ++i) {
		double e = fabs(D_direct - D_fast) / Q;
		if (e > max_error) {
			max_error = e;
		}
	}
